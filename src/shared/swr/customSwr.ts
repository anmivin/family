...
import React, { useCallback, useDebugValue, useMemo, useRef } from 'react';
...
export const useSWRHandler = <Data = any, Error = any>(
  ...
  const [getCache, setCache, subscribeCache, getInitialCache] = createCacheHelper<
    Data,
    State<Data, any> & {
      ...
      try {
        ...
        if (!currentConfig.isPaused()) {
          ...
          if (shouldStartNewRequest && callbackSafeguard()) {
            ...
            if (shouldRetryOnError === true || (isFunction(shouldRetryOnError) && shouldRetryOnError(err as Error))) {
                ...
                currentConfig.onErrorRetry(
                  ...
                  () => {
                    ...
                    if (revalidators && revalidators[0]) {
                      revalidators[0](revalidateEvents.ERROR_REVALIDATE_EVENT);
                    }
                  },
                  {
                    retryCount: 1,
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    const softRevalidate = revalidate.bind(UNDEFINED);
  ...
  if (suspense && isUndefined(data) && key) {
    ...
    if (isUndefined(error)) {
      const promise: ReactUsePromise<boolean> = revalidate();
